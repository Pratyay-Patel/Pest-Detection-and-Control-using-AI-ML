...
CONFIG = {
    ...
    "learning_rate": 3e-4,
    "weight_decay": 1e-4,
    "num_workers": 0,   # set to 0 on Windows to avoid DataLoader issues
...
if not os.path.exists(CONFIG["cache_file"]):
    raise RuntimeError(f"❌ Cached dataset not found: {CONFIG['cache_file']}\n"
                       f"Please make sure pest_dataset.pkl is in the folder.")
else:
...
# Label mappings
crop2idx = {lbl[0]: lbl[0] for lbl in labels}
pest2idx = {lbl[1]: lbl[1] for lbl in labels}
...
def train():
    ...
    # Model
    num_crops = len(set(l[0] for l in labels))
    num_pests = len(set(l[1] for l in labels))
